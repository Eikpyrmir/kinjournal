...
- `src/webdav.ts` — Nextcloud WebDAV バックアップ。キー `kinjournal.nextcloud.v1`（接続設定・平文、`enabled` で自動実行を制御）、`kinjournal.lastBackup.v1`。`kinjournal-backup.json` をルート直下に PUT（上書き）。起動時1日1回の自動実行は App.tsx。ブラウザから直接接続するため Nextcloud 側に CORS（`cors.allowed-domains`）設定が必須。復元機能はない
